...
import {issueSummeryReport,issueDetailsReport} from "../store/actions/Issues";
import {
	errorNotification,
	successNotification,
...
function GenerateissuesReport({
	...
	const generateReport = () => {
		const post = {
			issues: props.multiSelect,
			issue_id: props.multiSelect,
			project_id: props.project_id,
			user_id: userId,
		};
		if (reportType === 'summary') {
			setLoading(true);
			dispatch(issueSummeryReport(post, handleReportResponse));
		} else {
			setLoading(true);
			dispatch(issueDetailsReport(post, handleReportResponse));
		}
	};
						...
						<div className="row">
							<div className={`col-12`}>
								...
								<div className="fw-bold text-center">
									<h4>
										{/* {share_file_heading?.text} */} Did you select the report
										type?
									</h4>
								</div>
...
export default GenerateissuesReport;
