...
function CreatePlan(props) {
	...
	const { add_location, add_plan,plan_name } = getSiteLanguageData('task/update');
		...
		<>
			...
			<Modal show={show} onHide={handleClose} size="md" animation={false}>
				<Modal.Header closeButton>
					<Modal.Title>
						<h3 className="ms-5 theme-color">{add_location?.text}</h3>
					</Modal.Title>
				</Modal.Header>
				<Modal.Body>
					<div>
						<Form onSubmit={submitPlan}>
							...
							<Button type="submit" className="theme-btn mx-2 btn-block">
								{add_plan?.text}
							</Button>
						</Form>
					</div>
