import { Component } from 'react';
import { connect } from 'react-redux';

import Nodata from '../../components/nodata';
import Layout from '../../components/layout';
import {
	GET_ALL_EQUIPMENT_LIST,
	GET_ALL_EQUIPMENT_LOG_LIST_BY_ID_AND_DATE,
} from '../../store/actions/actionType';
import {
	downloadLabourAndEquipmentLog,
	getAllEquipmentList,
	getAllEquipmentLogByIdAndDate,
	signLabourAndEquipmentLog,
	updateEquipmentLog,
	deleteEquipmentLog
} from '../../store/actions/projects';
import { FormControl } from 'react-bootstrap';
import CustomSelect from '../../components/SelectBox';
import moment from 'moment';
import CreateEquipment from './createEquipment';
import CreateEquipmentLog from './createEquipmentLog';
import getUserId, { getSiteLanguageData, sweetAlert } from '../../commons';
import { getParameterByName } from '../../helper';
import { unlockReport } from '../../store/actions/report';
import ReportInfo from '../Reports/Components/reportInfo';
import { errorNotification } from '../../commons/notification';
import DatePicker from 'react-datepicker';
import Signature from '../../components/signature';
import withRouter from '../../components/withrouter';
import LabourAndEquipmentDetails from '../Labour/labourAndEquipmentDetails';
import CustomDate from '../../components/CustomDate';
import GenerateSurveyReport from '../Reports/Components/GenerateSurveyReport';
import { Link } from 'react-router-dom';

class EquipmentLog extends Component {
	constructor(props) {
		super(props);
		this.userId = getUserId();
		const { data } = this.props;
		this.project_id = this.props.router?.params.project_id;
		this.equipment_log_id = this.props.router?.params.labour_equipment_log_id;
		this.date = getParameterByName('labour_equipment_log_date');
		this.report_name_log = getParameterByName('name');
		this.state = {
			sortType: '3',
			info: data?.equipmentRs?.map((m) => {
				return {
					equipment_log_id: m?._id,
					user_id: this.userId,
					quantity: m?.quantity,
					hours: m?.hours,
					minutes:m?.minutes,
					project_id:m?.project_id,
					notes: m?.notes,
					equipment_id: m?.equipment_id,
					total_hours: m?.total_hours,
				};
			}),
			editList: [],
			editReport: false,
			signEquipment: {
				labour_equipment_log_id: this.equipment_log_id,
				user_id: this.userId,
				project_id: this.project_id,
				report_date: this.date,
				signature_url: '',
				signed_by: this.userId,
			},
			show: false,
			showShareModel:false,
			shareLink:null
		};
	}
	componentDidMount() {
		const { dispatch } = this.props;
		dispatch(getAllEquipmentLogByIdAndDate(this.equipment_log_id, this.date));
		dispatch(getAllEquipmentList(this.project_id));
	}

	componentDidUpdate(prevProps, prevState) {
		const { data } = this.props;
		if (data?.equipmentRs !== prevProps.data?.equipmentRs) {
			if (!!data?.equipmentRs) {
				this.setState({
					info: data?.equipmentRs?.map((m) => {
						return {
							equipment_log_id: m?._id,
							user_id: this.userId,
							quantity: m?.quantity,
							hours: m?.hours,
							minutes:m?.minutes,
							project_id:m?.project_id,
							notes: m?.notes,
							equipment_id: m?.equipment_id,
							total_hours: m?.total_hours,
						};
					}),
				});
			}
		}
	}

	hendleShowShereModel = () => {
		this.setState({showShareModel:!this.state.showShareModel});
	};

	setInfo = (info) => {
		this.setState({ info });
	};
	setSignEquipment = (signEquipment) => {
		this.setState({ signEquipment });
	};
	handleClose = () => {
		this.setState({ show: false });
	};
	handleChange = (name, value) => {
		this.setSignEquipment({
			...this.state.signEquipment,
			[name]: value,
		});
	};

	handleSetUrl = (url) => {
		this.setSignEquipment({
			...this.state.signEquipment,
			"signature_url": url,
		});
	}

	handleChangeEquipment = (name, val, k) => {
		if(name == "hours" && ( val != "" && (val == 0 || Number(val) <= 0 || Number(val) > 24))){
			errorNotification("Hours value should be between 0 to 24");
		}else if(name == "minutes" && ( val != "" && (val == 0 || Number(val) <= 0 || Number(val) > 60))){
			errorNotification("Hours value should be between 0 to 60");
		}else{
			const arr = this.state.info;
			const editList = [...this.state.editList];
			if (!editList.includes(arr[k].equipment_log_id)) {
				editList.push(arr[k].equipment_log_id);
			}
			arr[k] = {
				...arr[k],
				[name]: val,
			};

			this.setState({
				info: [...arr],
				editList,
			});
		}
		
	};

	handleSortType = (sortType) => {
		this.setState(sortType);
	};
	submitEquipment = (e) => {
		e.preventDefault();
		if(this.state.signEquipment && this.state.signEquipment.signature_url)
		this.handleClose();
		this.props.dispatch(signLabourAndEquipmentLog(this.state.signEquipment));
	};
	submitEditequipment = (e) => {
		e.preventDefault();
		const { info, editList } = this.state;
		const listToUpdate = info.filter((log) =>
			editList.includes(log.equipment_log_id),
		);
		for (let i = 0; i < listToUpdate.length; i++) {
			this.props.dispatch(
				updateEquipmentLog(listToUpdate[i], {
					labour_equipment_log_id: this.equipment_log_id,
					date: this.date,
					dispatchOthers: i == listToUpdate.length - 1,
					showMessage: i == listToUpdate.length - 1,
				}),
			);
		}
		this.setState({ editReport: false });
	};
	render() {
		const { data, equipment } = this.props;
		const equipmentLog = equipment?.map((b) => {
			return { label: b.name, value: b._id };
		});
		const { info, editReport, signEquipment } = this.state;
		// if (!data?.equipmentRs?.length && data?.equipmentRs?.length !== 0) {
		//   return <Loading />
		// }
		const {
			btn_download,
			btn_unlock,
			icon_edit,
			icon_select,
			icon_save,
			labour_equipment_log,
			btn_equipment,
			ph_selectEquipment
		} = getSiteLanguageData('equiqment');
		const { btn_Labour } = getSiteLanguageData('labour');
		const { description, nos, hours, minutes, total_hours, notes, action, nos_text } =
			getSiteLanguageData('commons');

		return (
			<Layout>
				<div id="page-content-wrapper">
					<section className="lf-dashboard-toolbar z-index-6">
					<section>
						<div className="row align-items-center">
							<div className="col-12">
								<div className="d-flex align-items-center">

									<div className="float-start d-none d-md-inline-block">
										<a
											className="lf-common-btn"
											href={`/reports/${this.project_id}`}>
											<i className="fa fa-arrow-left" aria-hidden="true"></i>
										</a>
									</div>
									<div className="float-start d-none d-lg-inline-block">
										<span className="text-nowrap">
											{this.report_name_log ? this.report_name_log : ''}
										</span>
									</div>

									<div className="ms-auto d-flex float-end align-items-center d-inline-block">

										<div className="float-start d-inline-block">
											<span className="pe-2 theme-secondary">
												<i
													className="fas fa-less-than"
													onClick={() => {
														const tomorrow = moment(this.date).toDate();
														const date = tomorrow.setDate(tomorrow.getDate() - 1);
														window.location.href = `/reports/${
															this.project_id
														}/equipment_log/${
															this.equipment_log_id
														}?labour_equipment_log_date=${moment(date).format(
															'YYYY-MM-DD',
														)}&name=${this.report_name_log}`;
													}}
												/>
												<span
													className="theme-btnbg mx-1 react-datepicker__input-container text-nowrap theme-secondary rounded lf-link-cursor"
													style={{
														display: 'inline-block',
														width: 'auto',
													}}>
													<DatePicker
														tooltip="Calender"
														flow="down"
														className="d-inline-block"
														selected={moment(this.date).toDate()}
														customInput={
															<span>
																<i className="far fa-calendar-alt" />{' '}
																<CustomDate date={this?.date} />
															</span>
														}
														onChange={(e) => {
															const date = moment(e).format('YYYY-MM-DD');
															window.location.href = `/reports/${this.project_id}/equipment_log/${this.equipment_log_id}?labour_equipment_log_date=${date}&name=${this.report_name_log}`;
														}}
														maxDate={moment().toDate()}
													/>
												</span>
												<i
													className="fas fa-greater-than"
													onClick={() => {
														const tomorrow = moment(this.date).toDate();
														const date = tomorrow.setDate(tomorrow.getDate() + 1);
														if (this.date < moment(new Date()).format('YYYY-MM-DD')) {
															window.location.href = `/reports/${
																this.project_id
															}/equipment_log/${
																this.equipment_log_id
															}?labour_equipment_log_date=${moment(date).format(
																'YYYY-MM-DD',
															)}&name=${this.report_name_log}`;
														} else {
															errorNotification("You Can't View Feature Report");
														}
													}}
												/>
											</span>
										</div>

										<div className="float-start d-none d-md-inline-block">
											<LabourAndEquipmentDetails
												data={this.equipment_log_id}
												type="Labour & Equipment Log"
												onlyinfo={"onlyinfo"}
											/>
										</div>

										<div className="float-start d-inline-block">
											{data?.equipmentlogreportRs?.map((c) => c?.is_locked)[0] ===
											true ? (
												<span
													className="lf-common-btn"
													onClick={() =>
														sweetAlert(
															() =>
																this.props.dispatch(
																	unlockReport({
																		report_date: this.date,
																		project_id: this.project_id,
																		labour_equipment_log_id: this.equipment_log_id,
																		user_id: this.userId,
																		report_name: this.report_name_log,
																		report_id: data?.equipmentlogreportRs?.map(
																			(c) => c?._id,
																		)[0],
																		report_type: 'LabourEquipmentLog',
																	}),
																),
															'Labour & Equipment Log',
															'Unlock',
														)
													}>
													<i className="fa-solid fa-lock-open pe-2"></i>
													{btn_unlock?.text}
												</span>
											) : (
												// <SignLabourAndEquipmentLog />
												<Signature
													setUrl={this.handleSetUrl}
													url={signEquipment?.signature_url}
													signReport={this.submitEquipment}
												/>
											)}
										</div>

										<div className="float-start d-inline-block">
											{data?.equipmentlogreportRs?.map((c) => c?.is_locked)[0] ===
											true ? (
												<span
													type="submit"
													className="lf-common-btn"
													onClick={() =>{
															this.setState({shareLink:null});
															sweetAlert(
																() =>{
																	this.hendleShowShereModel();
																	this.props.dispatch(
																		downloadLabourAndEquipmentLog({
																			project_id: this.project_id,
																			user_id: this.userId,
																			report_date: this.date,
																			labour_equipment_log_id: this.equipment_log_id,
																		},(repData)=>{
																			if(repData?.data?.result?.file){
																				this.setState({shareLink:repData?.data?.result?.file})
																			}else{
																				this.setState({shareLink:null})
																			}
																		}),
																	)},
																'Equipment Report',
																'Download',
															)
														}
													}>
													<i className="fa-solid fa-download pe-2"></i>
													{btn_download?.text}
												</span>
											) : (
												''
											)}
										</div>
									</div>	
								</div>
							</div>
						</div>
					</section>
					<div className='border-top'></div>
					<section>
						<div className="row align-items-center">
							<div className="col-12 my-2">
								<span className="px-4 border-0">
								<Link
									className="text-dark"
									to={`/reports/${this.project_id}/labour_log/${this.equipment_log_id}?labour_equipment_log_date=${this.date}&name=${this.report_name_log}`}>
									{btn_Labour?.text}
								</Link>
								</span>
								<span className='border-start py-0'></span>
								<span className="px-4 theme-color">
								{btn_equipment?.text}
								</span>								
							</div>
						</div>
					</section>

					</section>
				
					<section>
						<div className="px-3">
							<div className="row">
								{/* <div className="col-2 mt-1">
                  <i className="fas fa-search theme-btnbg py-2 m-1"></i>
                  <input
                    type="text"
                    className="d-block form-control border border-0"
                    placeholder="Search"
                  />
                </div> */}
								{/* <div className="col-4">
                {/* <Dropdown>
                    <Dropdown.Toggle variant="transparent" id="dropdown-basic" className="mt-1 lf-common-btn">
                      <span className="">{sortingList[parseInt(this.state.sortType) - 1]}</span>
                    </Dropdown.Toggle>
                    <Dropdown.Menu style={{ backgroundColor: '#73a47' }} className="shadow p-2 mb-2 bg-white rounded-7 dropdown-menu ">
                      {sortingList.map((st, k) => {
                        return <Dropdown.Item key={k} className='lf-layout-profile-menu' onClick={() => this.handleSortType((k + 1).toString())}>{st}</Dropdown.Item>
                      })}
                    </Dropdown.Menu>
                  </Dropdown>
              </div> */}
								<div
									className="col-12"
									style={
										data?.equipmentlogreportRs?.map((c) => c?.is_locked)[0] ===
										true
											? { display: 'none' }
											: { visibility: 'initial' }
									}>
									<CreateEquipment />
									<CreateEquipmentLog className="lf-link-cursor lf-main-button float-end" />

									{!info ? (
										''
									) : (
										<>
											{
												this.state?.editReport == true ? (
													<span
														type="submit"
														className="p-2 btn theme-btnbg theme-secondary rounded float-end"
														onClick={this.submitEditequipment}>
														{icon_save?.text}
													</span>
												) : (
													<span
														type="submit"
														onClick={() => this.setState({ editReport: true })}
														className="p-2 btn theme-btnbg theme-secondary rounded float-end">
														{icon_edit?.text}
													</span>
												)
											}
											{/* <span
												type="submit"
												className="p-2 btn theme-btnbg theme-secondary rounded float-end">
												{icon_select?.text}
											</span> */}
											
											
											
										</>
									)}
								</div>
							</div>
						</div>
					</section>
					<div className="mx-2 mt-3">
						{!data?.equipmentRs ? (
							<Nodata type="equipment log">
								<CreateEquipmentLog className="lf-link-cursor lf-main-button" />
							</Nodata>
						) : (
							<div className="theme-table-wrapper card mx-3">
								{/* <div className="theme-table-wrapper theme-table-wrapper-180 lf-w-1000 card"> */}
								<table className="table table-hover theme-table text-center">
									<thead className="theme-table-title text-nowrap bg-light">
										<tr className="bg-light text-nowrap text-capitalize col-12">
											<th className="col-4 text-start ps-3">
												{description?.text}
											</th>
											<th className="col-1 text-center">{nos?.text}</th>
											<th className="col-1 text-center">{hours?.text}</th>
											<th className="col-1 text-center">{minutes?.text}</th>
											<th className="col-1 text-center">{total_hours?.text}</th>
											<th className="col-4 text-center">{notes?.text}</th>
											<th className="col-1 text-center">{action?.text}</th>
										</tr>
									</thead>
									<tbody>
										{
											// data?.equipmentRs?.sort((a, b) => {
											//   if (state.sortType === "1") {
											//     return a.name?.charCodeAt(0) - b.name?.charCodeAt(0);
											//   }
											//   if (state.sortType === "2") {
											//     return b.name?.charCodeAt(0) - a.name?.charCodeAt(0);
											//   }
											//   if (state.sortType === "3") {
											//     return new Date(b.createdAt) - new Date(a.createdAt);
											//   }
											//   if (state.sortType === "4") {
											//     return new Date(a.createdAt) - new Date(b.createdAt);
											//   }
											//   return true;
											// })
											info?.map((p, k) => {
												console.log(p, "p p")
												return (
													<tr key={k} className="col-12 align-middle">
														<td className="col-4 text-start ps-3 align-top">
															{!editReport ? (
																equipmentLog?.filter(
																	(as) => as.value === p?.equipment_id,
																)[0]?.label
															) : (
																<CustomSelect
																	className=""
																	placeholder={ph_selectEquipment?.text}
																	name="equipment_id"
																	onChange={(e) =>
																		this.handleChangeEquipment(
																			'equipment_id',
																			e.value,
																			k,
																		)
																	}
																	options={equipmentLog}
																	value={equipmentLog?.filter(
																		(as) => as.value === p?.equipment_id,
																	)}
																	// isDisabled={data?.equipmentlogreportRs?.map(c => c?.is_locked)[0] === true}
																	// isDisabled
																/>
															)}
														</td>
														<td className="col-1 text-center align-top">
															<span>
																{!editReport ? (
																	p?.quantity
																) : (
																	<FormControl
																		className="lf-h-37 text-center"
																		type="number"
																		autoComplete="off"
																		onChange={(e) =>
																			this.handleChangeEquipment(
																				'quantity',
																				e.target.value,
																				k,
																			)
																		}
																		placeholder={'Nos.'}
																		value={p?.quantity}
																		disabled={
																			data?.equipmentlogreportRs?.map(
																				(c) => c?.is_locked,
																			)[0] === true
																		}
																	/>
																)}
															</span>
														</td>
														<td className="col-1 text-center align-top">
															<span>
																{!editReport ? (
																	p?.hours
																) : (
																	<FormControl
																		className="lf-h-37 text-center"
																		type="number"
																		autoComplete="off"
																		onChange={(e) =>
																			this.handleChangeEquipment(
																				'hours',
																				e.target.value,
																				k,
																			)
																		}
																		placeholder={'Hours'}
																		value={p?.hours}
																		disabled={
																			data?.equipmentlogreportRs?.map(
																				(c) => c?.is_locked,
																			)[0] === true
																		}
																	/>
																)}
															</span>
														</td>
														<td className="col-1 text-center align-top">
															<span>
																{!editReport ? (
																	p?.minutes
																) : (
																	<FormControl
																		className="lf-h-37 text-center"
																		type="number"
																		autoComplete="off"
																		onChange={(e) =>
																			this.handleChangeEquipment(
																				'minutes',
																				e.target.value,
																				k,
																			)
																		}
																		placeholder={'Minutes'}
																		value={p?.minutes}
																		disabled={
																			data?.equipmentlogreportRs?.map(
																				(c) => c?.is_locked,
																			)[0] === true
																		}
																	/>
																)}
															</span>
														</td>
														<td className="col-1 text-center align-top">
															<span>
																{!editReport ? (
																	p?.total_hours
																) : (
																	<FormControl
																		className="lf-h-37 text-center"
																		type="number"
																		autoComplete="off"
																		placeholder={'Hours'}
																		value={p?.total_hours}
																		disabled
																	/>
																)}
															</span>
															{/* 	<span>{p?.total_hours}</span> */}
														</td>
														<td className="col-4 ps-3 text-start align-top">
															<span>
																{!editReport ? (
																	p?.notes
																) : (
																	<FormControl
																		className=""
																		type="text"
																		autoComplete="off"
																		onChange={(e) =>
																			this.handleChangeEquipment(
																				'notes',
																				e.target.value,
																				k,
																			)
																		}
																		placeholder={'Notes'}
																		value={p?.notes}
																		disabled={
																			data?.equipmentlogreportRs?.map(
																				(c) => c?.is_locked,
																			)[0] === true
																		}
																	/>
																)}
															</span>
														</td>
														<td className="col-1 text-center align-top">
															{/* <UpdateEquipmentLog data={p} /> */}
															<span
																className="p-2 theme-btnbg theme-secondary rounded lf-link-cursor theme-btnbold"
																style={
																	data?.equipmentlogreportRs?.map(
																		(c) => c?.is_locked,
																	)[0] === true
																		? { pointerEvents: 'none', opacity: '0.7' }
																		: { opacity: '1' }
																}
																onClick={() => {

																	sweetAlert(
																		() =>{
																			this.props.dispatch(
																				deleteEquipmentLog({
																				  equipment_log_id: [p?.equipment_log_id],
																				  labour_and_equipment_log_id:this.equipment_log_id,
																				  date: this.date,
																			  }))
																		},
																		'Equipment log',
																		'Delete',
																	)

																//   const isConfirmDelete = window.confirm(`are you sure to Delete equipment`)
																//   if (isConfirmDelete) {
																    
																//   }
																}}
															>
																<i className="fas fa-trash-alt"></i>
															</span>
														</td>
													</tr>
												);
											})
										}
									</tbody>
								</table>
							</div>
						)}
					</div>
				</div>
				<GenerateSurveyReport open={this.state.showShareModel} project_id={this.project_id} shareLink={this.state.shareLink} handleClose={this.hendleShowShereModel} />
			</Layout>
		);
	}
}

export default withRouter(
	connect((state) => {
		return {
			data:
				state?.project?.[GET_ALL_EQUIPMENT_LOG_LIST_BY_ID_AND_DATE]?.result ||
				[],
			equipment: state?.project?.[GET_ALL_EQUIPMENT_LIST]?.result || [],
		};
	})(EquipmentLog),
);
